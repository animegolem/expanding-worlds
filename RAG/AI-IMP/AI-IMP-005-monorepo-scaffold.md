...
- [ ] pnpm workspace + root scripts (build, test, lint) resolve and run.
- [ ] Six stub packages with strict tsconfig references build via `pnpm -r build`.
- [ ] One vitest test per package passes via `pnpm -r test`.
- [ ] eslint flat config green; boundary fixture importing @ew/persistence from a renderer path fails lint (verified, then fixture removed).
- [ ] Clean-checkout verification: fresh clone into temp dir installs and passes all gates.
