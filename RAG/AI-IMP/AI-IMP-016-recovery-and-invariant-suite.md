...
- [ ] recovery.ts: ordered checks per §11.4 with a structured report (checks run, repairs made, integrity errors); unit tests per check with seeded-fault fixtures.
- [ ] Pending-import reconciliation: fixtures for each pending state (staging, hashed) reconcile to clean temp dirs and no dangling records; unreferenced orphan blobs removed only when age + no-reference checks pass (§9.8).
- [ ] Missing canonical original produces a visible integrity error in the open result; missing thumbnail only enqueues regeneration (test both).
- [ ] Cross-process lock test: second OS process fails to acquire the project and receives the structured lock error; lock released on clean close is acquirable (epic metric 3).
- [ ] Kill-during-import: child process killed between hash and commit; reopen recovers with no Asset row, no temp files, blob either absent or eligible-orphan-collected (epic metric 4).
- [ ] gc.ts guards use real pending-import and derivative-job queries; export-lease stub documented for EPIC-008.
- [ ] invariants.spec.ts: one describe per §5 rule 1–31, each asserting through the public service surface; rules 24–25 and 29–31 tested at data-contract level (inverse round-trip, one command_log row per gesture-committed command, metadata log survives reopen, no undo tables exist in schema).
- [ ] Representative §10.1 command sequence test: scripted run of every implemented command type with monotonic revision assertions and a stale-revision conflict (epic metrics 1–2).
- [ ] Full `pnpm check` green on merged master; epic FR checkboxes checked with justification; epic closed (kanban, date_completed), `./RAG/scripts/generate-index.sh` run, AI-LOG entry written; commit.
