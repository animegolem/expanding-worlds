...
label never sits, so the position itself is the tell — with the
sort-state chip beside it (grid · rows · float ▾; "float" is the
visible off-state). Furniture is zoom-gated by the shrink ladder
(exists only above EW_FURNITURE_MIN_PX-derived threshold; the
region keeps a ≥1px stroke so membership never vanishes). Done
means: a titled frame shows its on-edge label + sort chip above
the threshold, the chip reflects and sets the AI-IMP-129 sort
state, and nothing renders below the threshold except the minimum
stroke.

### Out of Scope

- The sort-control LOCATION final call (chip vs charm bar —
  design-queue): the chip ships as drawn; if the queue resolves to
  charm-bar, the chip retires then.
...
Chip states read placement presentation (129's
`frame_sort_on_drop` + a sort-mode fact if 129 stores one; else
chip = On/float toggle + sort-now action matching the Dock's).
Zoom gate via 133's helper; the ≥1px region stroke is engine-side
(placement renderer minimum). Interactions: click chip → cycle/
popover per the drawn 1j direction; changes dispatch the same
actions as the Dock buttons (no new commands).
...
- [ ] On-edge mono title + sort chip, tokens only, engagement
      cadence, never in exports (adornment layer).
- [ ] Zoom gate via the shared constant; region stroke ≥1px
      always (unit).
- [ ] Chip reflects + sets sort state (same action path as Dock);
      e2e round-trip.
...
**THEN** its top edge carries the mono title and a "grid ▾"-style
chip that toggles the 129 behavior
**AND** below the furniture threshold both vanish while the
