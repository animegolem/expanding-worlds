...
6. **The shell eats the window (→ RFC §8 shell layout + Design
   Spec §1 — gap found 2026-07-07; build lands in the .3 release).**
   Every drawing we have shipped (UI Vision v1/v2, all seven hero
   states, the wireframe winner) paints traffic lights directly on
   the board — but no document ever says the load-bearing sentence,
   and the live app still shows a native titlebar. AI-IMP-074 even
   cites "(frameless work)" as if ratified elsewhere; this entry is
   that elsewhere. Proposed wording:

   - **There is no titlebar.** The window is frameless on every
     platform; the board paints to all four edges. The OS never
     draws a bar, ever — not in fullscreen transitions, not on
     Win/Linux.
   - **The traffic lights are drawn into the board,** upper-left,
     with the §8.1 path beside them (macOS: native buttons inset
     via the frameless window's position hook — native behaviors
     kept; Win/Linux: our drawn controls, living in the title
     strip). Open feel question: whether the lights join the one
     fade clock or rest exempt at full opacity.
   - **The hover-revealed title strip IS the drag handle.** The
     §8 strip (file/view functions, system menus on Win/Linux)
     doubles as the window-move drag region: the top-edge reveal
     zone is hot for dragging even before the strip is visible;
     controls inside it are no-drag islands. This closes the
     until-now-unanswered "how do you move a frameless window
     whose whole surface pans the canvas."
   - **Hold-to-move / hold-to-resize, anywhere (the Linux super-key
     reflex).** A held modifier makes the entire window a handle,
     no strip required: hold + drag anywhere = move window; hold +
     secondary-drag (or hold near an edge) = resize. The modifier
     is what disambiguates window management from canvas panning.
     Binding is an open feel choice per platform (Mod4/Super is
     the native reflex on Linux; macOS needs a chord that clears
     ⌘-drag duplicate and the tool set); it registers in the §
     keyboard registry and prints in settings like every binding.
   - Tooltip/registry rule applies: the strip and the hold-chord
     are taught by the GUI (strip tooltip names the drag; the
     keyboard sheet prints the chord). Nothing here reflows the
     canvas; the strip stays an overlay on the one fade clock.

7. **App icon crowned + the canonical pin (App Icon Document,
   2026-07-07 — owner).** The mark is 5a: the world-photo reference
   card, cross-angled, pinned by the carved-red teardrop on
   silhouette iv (concave swooped sides, shortened rounded tip).
   Flat-mono pin treatments were retired on doctrine grounds — the
   pin is a desk object. PROPOSED, riding this: silhouette iv
   becomes THE canonical pin drawn everywhere "pins mean places"
   (path-tail bookmark, dock ◉ tool, the object push pin's
   map-pin contexts), replacing the ad-hoc 50/50/50/0 teardrop.
   OPEN (owner musing, to wireframe next): the red pin as the ONLY
   colored element in the mono chrome — colored in the dock, on
   the path-tail bookmark, or both — as the app's one signature
   color mark. Needs the chrome-is-a-terminal exception ratified
   if taken.
   RESOLVED (owner, at the shell screenshot): the path-tail
   bookmark IS the signature spot — it is the app's only bookmark
   control, so the red canonical pin lives there alone; the dock
   ◉ tool stays mono. Doctrine note to ratify with it: the pin is
   the ONE isomorphic bridge between flat chrome and the physical
   canvas — it literally pushes into the canvas — the single
   sanctioned crossover of the two materials.

8. **The bookmark-open beat + a universal menu grammar (to
   prototype next pass).** (a) Bookmark click: the pin does an
   anticipation wiggle (a dog readying its hop), pops UP off the
   bar, and pushes DOWN into the canvas with a baby touch of
   squash-and-stretch — quick but a little lavish, one-shot,
   world-beat budget; the bookmarks menu + new-bookmark row then
   animate in UNDERNEATH the pinned pin, anchored to the opener
   per the one physics rule. (b) ALL menus get one shared
   open animation: a directional opacity sweep — genie-flavored
   but not exaggerated, a fade-in with direction from the anchor.
   2–3 versions to try. Chrome-animates-only-opacity survives:
   the sweep is opacity with a moving mask; only the pin moves.
...
## Interaction physics (Interaction Physics Document t1 — sprint 3 opener)

- ABSORBED into the canonical docs: UI Vision v2 (header names the
  six verbs; 01 shows one item mid-lift with drag shadow; 02's
  bound page is now FLAT per shadow-follows-glass) and Usage
  Storyboard (§2 lift/settle/nudge · §6 book-open flat + full
  three-state lifecycle w/ reversibility · §10 making-room ·
  §16 delete-lifts-away + burn-on-purge).

- Six physical verbs cover every canvas mouse-down: **lift ·
  settle · nudge · press · tear · glide** — one-shot, world
  content only. Grab = lift (drag shadow, +1%, ~120ms); release =
  settle (~150ms, no bounce); import drop = settle + stage bloom;
  snap = nudge (~40ms magnetic seat, Option slips); lock = press
  (−1% into the desk; grab attempt = 2px strain, not motion);
  delete = lift AWAY (~180ms — NEVER a crumple: the trash keeps
  things whole, a crumple would lie); pan flick = glide (mass +
  friction, ≤300ms, no rubber band); double-click dive = the §6.5
  flight; frame hover = making room (members slide to open the
  real seat — the one anticipatory motion, because it previews
  truth). Deliberate no-beats: resize/rotate/crop mid-use (live
  manipulation is its own feedback), marquee/select/draw (chrome
  doesn't emote).
- Build contract: EW_BEAT_LIFT_MS ~120 · SETTLE ~150 · NUDGE ~40 ·
  TAKE ~180 · GLIDE_MAX ~300; ±1% scale only; beats interruptible;
  guard — beats only on world-class elements, chrome animates
  opacity only.
- **Animated (t2):** universal grab loop, lock strain, delete
  lift-away, and the **burn** (Compiz/Beryl wink — owner ask).
  Pairing rule taken: delete = lift-away (thing survives; beat
  must not say "destroyed"); **empty trash/purge = burn** (~450ms,
  the one honest fire; mask wipe + ember edge, no particle
  engine). Owner may override to burn-on-delete.
...
- **The Two Materials** — DONE (The Two Materials.dc.html): the
  aestheticized North Star page merging Interaction Physics +
  Note Lifecycle, built for repo onboarding beside the Rulebook.
  Hero states the doctrine; LAYER 01 the terminal (glyph/chip/
  tooltip specimens, a live one-clock fade demo, "UX-layer
  feature = you are working in mono"); LAYER 02 the desk
  ("canvas-level feature = you are working in physics", the
  three-state lifecycle as one looping tear→tape→pin animation
  with state captions, plus grab/press/burn beat loops); footer =
  the three-question material test (own it · visible event ·
  would the metaphor lie).

- **Desk or Terminal Rulebook** — DONE (Desk or Terminal
  Rulebook.dc.html): the repo-onboarding when-skeuo/when-flat
  doctrine doc, PR-review framing — the three-question test
  (ownership · state-legibility · does-the-metaphor-lie), full
  desk/terminal catalog of every example in the app, decided
  border cases (switcher, source panel, shadow-follows-glass,
  frames-as-furniture), the forbidden-moves checklist, and the
  amendment route. Lives beside STYLE-GUIDE.md in the repo.
