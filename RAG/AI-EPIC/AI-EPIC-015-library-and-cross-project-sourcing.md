...
- [x] FR-1: Library project designation (packaging, not schema) and the gallery's this world · everything scope toggle per §14.4 (rev 0.22).
- [x] FR-2: Read-only source-project opening per §11.1/§14.4.
- [x] FR-3: Ingest-by-copy with provenance and the tag border decision (none/all/pick, name_key merge) per §14.4.
- [x] FR-4: Placement picker as compressed gallery; sources open as pinned mini-gallery panels from the project charm per §14.4.
