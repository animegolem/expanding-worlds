...
describe('kit input component contracts', () => {
  ...
  it('keeps the kit toggle semantic, focused, and comfortable', () => { const source = read('./Toggle.svelte'); expect(source).toContain('role="switch"'); expect(source).toContain('aria-checked'); expect(source).toContain('outline: 2px solid var(--ew-focus-ring)'); expect(source).toContain("data-density='comfortable'"); expect(source).toContain('44px') })
