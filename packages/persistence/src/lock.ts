...
function reclaimUnderGuard(
  ...
  try {
    ...
    } else if (owner.kind === 'absent') {
      // Never unlink after observing absence: an O_EXCL winner may create a
      // fresh live owner between the read and a later unlink syscall.
      return { removed: true }
