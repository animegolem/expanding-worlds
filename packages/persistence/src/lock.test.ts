...
const faults = vi.hoisted(() => ({
  ...
  replaceLockOnNextReadWith: null as string | null,
  replaceLockOnReadNumber: 0,
  lockReadCalls: 0,
...
vi.mock('node:fs', async (importOriginal) => {
  ...
  return {
    ...
    readFileSync: (...args: Parameters<typeof actual.readFileSync>) => {
      if (String(args[0]).endsWith('project.lock')) faults.lockReadCalls++
      if (
        faults.replaceLockOnNextReadWith &&
        faults.lockReadCalls === faults.replaceLockOnReadNumber &&
        String(args[0]).endsWith('project.lock')
      ) {
        const replacement = faults.replaceLockOnNextReadWith
        faults.replaceLockOnNextReadWith = null
        actual.writeFileSync(args[0], replacement)
        const err = new Error('injected absent read after fresh winner') as NodeJS.ErrnoException
        err.code = 'ENOENT'
        throw err
      }
...
afterEach(() => {
  ...
  faults.replaceLockOnNextReadWith = null
  faults.replaceLockOnReadNumber = 0
  faults.lockReadCalls = 0
...
describe('ProjectLock', () => {
  ...
  it('never unlinks a fresh winner installed after an absent guarded read (AI-IMP-264)', () => {
    const corpse: LockHolder = {
      pid: 999999,
      hostname: 'ghost-host',
      token: 'corpse-token',
    ...
    const fresh: LockHolder = {
      pid: process.pid,
      hostname: hostname(),
      token: 'fresh-winner-token',
      acquiredAt: new Date().toISOString(),
      heartbeatAt: new Date().toISOString(),
    }
    writeFileSync(join(dir, LOCK_FILENAME), JSON.stringify(corpse))
    faults.replaceLockOnReadNumber = 2
    faults.replaceLockOnNextReadWith = JSON.stringify(fresh)

    expect(() => ProjectLock.acquire(dir, { staleAfterMs: 0 })).toThrowError(ProjectLockedError)
    expect(faults.lockUnlinkCalls).toBe(0)
    expect(JSON.parse(readFileSync(join(dir, LOCK_FILENAME), 'utf8'))).toMatchObject({
      token: 'fresh-winner-token',
    })
  })
